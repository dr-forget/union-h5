  ...
  <view class="custom-page">
    <view v-if="showNavBar" class="navbar" :style="navBarstyle">
      <view class="nav-center" :style="{ top: useAppConfig.systemConfig.appletInfo.top + 'px' }">
        <view v-if="slots.navbarLeft" class="navbarLeft" :style="{ height: useAppConfig.systemConfig.appletInfo.height + 'px' }">
          <slot name="navbarLeft" :right-meunwidth="useAppConfig.systemConfig.appletInfo?.width"></slot>
        </view>
        <view v-if="isback" class="navbarLeft" :style="{ height: useAppConfig.systemConfig.appletInfo.height + 'px' }" @click="backUp">
          <line-icon name="nine9-fanhui f-bold f-32 inline-block" :style="{ color: state.navigationColor }"></line-icon>
        </view>
        <view
        ...
        >
          <slot name="navbarCenter" :rightmeun-height="useAppConfig.systemConfig.appletInfo.height"></slot>
        </view>
        <view v-if="props.title" class="navbarCenter-title" :style="{ height: useAppConfig.systemConfig.appletInfo.height + 'px' }">
          <text v-if="state.opcity > 0.5" class="title u-line-1" :style="{ color: gradientFontColor }" @click="emit('click')">{{ title }} </text>
        </view>
        <view></view>
      </view>
    </view>
    ...
    >
      <u-skeleton :rows="useAppConfig.systemConfig.rows" :loading="state.pageLoading" :title="false">
        <slot></slot>
        <slot name="content" :navbar-height="state.iswechat ? 0 : useAppConfig.systemConfig.navbarHeight"></slot>
...
import { useSlots, ref, watch, onMounted, provide, CSSProperties, getCurrentInstance, computed, nextTick } from 'vue';
...
const { state, setData } = useState<DataType>({
  ...
  refresherEnabled: props.pullDownRefresh ? true : false, //是否开启刷新
...
const scrollHeight = computed(() => {
  ...
  if (props.isNavBar && !props.isFixed) {
    height = state.iswechat ? 0 : useAppConfig.systemConfig.navbarHeight;
  }
...
const scrollTop = (top?: number, timer?: number) => {
  ...
  // #endif
  // #ifdef H5
  const arrdom = document.getElementById('scrollRef')?.getElementsByClassName('uni-scroll-view') || [];
  const dom = arrdom[arrdom.length - 1];
  if (dom) {
    dom.scrollTo({ top: top || 0, behavior: 'smooth' });
  }
...
const onScroll = throttle((e: any) => {
  if (props.gradient) {
    let scroll = e.target.scrollTop <= 0 ? 0 : e.target.scrollTop || e.detail.scrollTop;
    const scrollH = uni.upx2px(state.scrollH);
    let opcity = scroll / scrollH;
