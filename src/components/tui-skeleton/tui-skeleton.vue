<template>
  <view class="tui-skeleton-cmomon tui-skeleton-box" catchtouchmove :style="{ width: winWidth + 'px', height: winHeight + 'px', backgroundColor: backgroundColor, position: position }">
    <view
      v-for="(item, index) in skeletonElements"
      :key="index"
      class="tui-skeleton-cmomon"
      :class="{ 'tui-skeleton__active': item.active && active }"
      :style="{
        width: item.width + 'rpx',
        height: item.height + 'rpx',
        left: item.left + 'rpx',
        top: item.top + 'rpx',
        backgroundColor: item.backgroundColor || skeletonBgColor,
        borderRadius: item.borderRadius || getRadius(item.skeletonType, borderRadius),
      }"
    >
    </view>
